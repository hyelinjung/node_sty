...
const parseCookie = (cookie = '')=> //쿠키의 값이 없을시 기본값 ''
    ...
    http.createServer((req,res)=>{
        ...
        if(req.url.startsWith('/login')){
            ...
            session[sessionId]={
               id, //변수명과 속성명이 같아 하나로 합침
               expriedDate,
            };
            res.writeHead(302,{
                Location: '/',
                'Set-Cookie': `name=${sessionId};Expires=${expriedDate.toUTCString()};HttpOnly;Path=/`,
            }); //쿠키는 설정할때 각종 옵션을 넣을 수 있고, 옵션간에는 ;로 구분 , 주의)쿠키 값에는 특정문자나 공백(중요!)등이 포함되면 오류가 발생
            res.end();
        }else if(cookie.name && session[cookie.name].expriedDate > new Date()){
            res.writeHead(200,{'Content-Type':'text/html; charset=utf-8'});
            res.end(`${session[cookie.name].id}님 안녕하세요`);
        }else{
